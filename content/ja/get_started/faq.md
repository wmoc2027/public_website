...
IOF Eventor を通じてエントリープロセスを完了し、エントリー料金の支払いが確認されると、WMOC2027 組織委員会はあなたを WMG に登録します。その後、WMGマイページへのアクセス、パスワードのリセット、必要書類の登録などの手順が記載されたメール通知が届きます。これに続いて他の競技に参加できるようになります。
このプロセスには通常数週間かかります。他のコンテストへのエントリーを優先したい場合は、まずご自身で WMG に登録してください。その後、[エントリーページの「WMGへのエントリーを優先された方」]({{% ref "get_started/entry" %}})のメールフォームより、WMG IDとIOF EventorアカウントIDをご入力ください。
  {{< /accordion-item >}}
{{< /accordion >}}
...
{{< accordion id="travel-accomodation" always-open="true" >}}
  {{< accordion-item title="WMOC 地区周辺で良い宿泊施設を見つけるにはどうすればよいですか?" >}}

WMOC およびその周辺地域では、さまざまなオプションが利用可能です。

最初の選択肢は、WMOCの主催自治体にとどまることだ。下記各自治体のホームページをご覧ください。なお、洋風のホテルや旅館は限られていますが、素敵な和風の旅館も見つかるかもしれません。一部の宿泊施設からはオプション料金のオフィシャルバスが運行されています。
